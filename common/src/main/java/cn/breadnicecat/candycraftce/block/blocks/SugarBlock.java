...
public class SugarBlock extends FallingBlock {
	...
	private static final Predicate<BlockState> IS_EMPTY = blockState -> blockState.isAir() || blockState.is(Blocks.LAVA) || blockState.is(CARAMEL_PORTAL.get());
			...
			for (BlockPos po : getNeighbourPos(pos)) {
				...
				if (IS_EMPTY.test(ps)) {
					...
					if (opt.isPresent()) {
						...
						shape.build(CARAMEL_PORTAL.get().defaultBlockState());
