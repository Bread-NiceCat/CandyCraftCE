...
public class CConfiguredFeaturesData {
	public static void bootstrap(BootstapContext<ConfiguredFeature<?, ?>> context) {
		...
		//TODO
		FeatureUtils.register(context, CANDIED_CHERRY_TREE, Feature.TREE, createFancyOakLike(MARSHMALLOW_LOG.get(), MAGIC_LEAVES.get()).build());
	}

	private static TreeConfiguration.TreeConfigurationBuilder createOakLike(Block log, Block leaves) {
		return createStraightBlobTree(log, leaves, 4, 2, 0, 2).ignoreVines();
	}
	...
	private static TreeConfiguration.TreeConfigurationBuilder createFancyOakLike(Block log, Block leaves) {
		return new TreeConfiguration.TreeConfigurationBuilder(
				...
				.ignoreVines()
				.dirt(BlockStateProvider.simple(PUDDING.get()));
	}
