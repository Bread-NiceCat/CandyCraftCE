...
//			CCEntityManager.SPAWN_EGG_HELPER.getEntries().forEach((k, v) -> withExistingParent(k.getEntryName(), "item/template_spawn_egg"));
			...
			accept(((Supplier<? extends Item> i) -> basicItem(i.get())),
					...
					SUGAR_CRYSTAL, COTTON_CANDY, GUMMY, GUMMY_BALL, HOT_GUMMY, CHOCOLATE_COIN,
...
//					BLACK_CHOCOLATE_BRICK,
					WHITE_CHOCOLATE_LEAF, JELLY_SENTRY_KEY, JELLY_BOSS_KEY, JELLY_DUNGEON_KEY,
					...
					HONEYCOMB_HELMET, HONEYCOMB_CHESTPLATE, HONEYCOMB_LEGGINGS, HONEYCOMB_BOOTS, LICORICE_SPEAR,
	...
	public ItemModelBuilder handheldItem(ItemEntry<?> item, @NotNull ResourceLocation tex) {
		return getBuilder(item.getName())
				.parent(HANDHELD)
				.texture("layer0", ResourceLocation.parse(tex.getNamespace() + ":item/" + tex.getPath()));
	}
