...
//			mapping(modLoc("block/" + CUSTARD_PUDDING.getName() + "_bottom"), blockTexture(PUDDING.get()));
		...
		//glass_pane * *_edge
		zone(() -> {
			mapping(blockTexture(CARAMEL_GLASS_PANE.get()), blockTexture(CARAMEL_GLASS));
			mapping(blockTexture(ROUND_CARAMEL_GLASS_PANE.get()), blockTexture(ROUND_CARAMEL_GLASS));
			mapping(blockTexture(DIAMOND_CARAMEL_GLASS_PANE.get()), blockTexture(DIAMOND_CARAMEL_GLASS));
...
//					of(BLACK_CHOCOLATE_FURNACE, BLACK_CHOCOLATE_STONE)
	...
	@Override
	public @NotNull ResourceLocation modLoc(@NotNull String name) {
		ResourceLocation loc = super.modLoc(name);
		return mappings.getOrDefault(loc, loc);
	}
	
	@Override
	public @NotNull ResourceLocation mcLoc(@NotNull String name) {
	...
	public ModelFile.ExistingModelFile existModelFile(Block block) {
		return existModelFile(BuiltInRegistries.BLOCK.getKey(block).withPrefix("block/"));
	}
	
	public ModelFile.ExistingModelFile existModelFile(ResourceLocation location) {
		return new ModelFile.ExistingModelFile(location, exFileHelper);
	}
