...
import cn.test.minecraft.utils.Int2ObjectPair;
...
import static cn.test.minecraft.Main.GSON;
...
public class BBModelParser {
	 ...
	 */
	public static JsonObject toCat(List<String> rawLines) {
		//行数->行
			...
			if (l.matches("PartDefinition .+? = .+?;")) {
				...
				if (l.startsWith("addOrReplaceChild(")) {
					...
					*/
					...
					List<Float> params = Utils.substringByRegex(s, "(-?\\d+\\.?\\d*F)").stream().map((Float::valueOf)).toList();
				...
				List<String> params = Utils.substringByRegex(l, "(?<=\\(|, ).+?(?=\\)|, )");
