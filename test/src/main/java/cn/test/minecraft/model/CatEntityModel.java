...
//	}
	...
	void validate() {
		Utils.assertTrue(texU * texV > 0, "illegal tex size: " + texU + "*" + texV);
	}
	...
	void addOrReplaceChild(String parent, String name, CubeListBuilder builder, PartPose partPose) {
		...
		if (parent != null) {
			Utils.assertTrue(!name.equals(parent), "Illegal quote: parent->this.name");
			Utils.assertTrue(boneNames.contains(parent), "Unknown parent: " + parent + " of " + name);
		}
		Utils.assertTrue(boneNames.add(name), "Duplicate bone name: " + name);
