...
import cn.breadnicecat.minecraft.model.BBModelParser;
import cn.breadnicecat.minecraft.model.anim.AnimLoader;
import cn.breadnicecat.minecraft.model.anim.ModelAnimation;
...
import static cn.breadnicecat.minecraft.utils.Utils.ifEndsWithOrAppend;
...
//		parseBBModel("bbmodel", true);
	...
	private static void parseBBModel(String source, boolean gz) throws IOException {
		try (InputStreamReader reader = new InputStreamReader(ResourceManager.getResource(source))) {
			...
			Path path = GENERATED_DIRECTORY.resolve(ifEndsWithOrAppend(source, gz ? ".json.gz" : ".json"));
			try (var writer = gz ?
